...
import { generatePassword, copyToClipBoard } from "../../tools/Gen";
...
const Display = () => {
  ...
  // Returns the colors based on the length of the password.
  const setBackgroundColor = (password) => {
    // If length is one to five characters, description "Bad password" returns red color
    if (password && password.length === 1 && password.length <= 5) {
      pwdDescription = "Bad password";
      return "#cb473e";
      // If length is six to ten characters, description "Weak password" returns yellow color
    } else if (password && password.length >= 6 && password.length <= 10) {
      pwdDescription = "Weak password";
      return "#f07d58";
      // Greater than 10, description "Strong password" returns green color
    } else if (password && password.length > 10) {
      pwdDescription = "Strong password";
      return "#55a95d";
      // Default message is Bad password background color of red.
    } else {
      pwdDescription = "Bad password";
      return "#cb473e";
    }
...
// Returns the colors based on the length of the password.
// Password length is greater than 10, the app displays a circle check icon
// Otherwise, it displays a different icon and description
  return (
    <>
      <div>
        <select
        ...
        >
          ...
          <option value="pin">Random PIN</option>
        </select>
      </div>
      <div className="row">
